...
public abstract class baseDistVisObj{
    ...
    private void _drawZeroLines(IGraphicsAppInterface ri) {
        ri.pushMatState();
        ri.setStrokeWt(2.0f);
        ri.setColorValFill(IGraphicsAppInterface.gui_Cyan,255);
        if (drawZeroAxes[0]) {//draw x==0 axis
            float yVal = zeroAxisVals[0];
            //draw line @ y Val
            ri.setColorValStroke(IGraphicsAppInterface.gui_White,255);
    ...
    protected void drawAxes(IGraphicsAppInterface ri, float offset) {
        ri.pushMatState();
        ri.setColorValFill(IGraphicsAppInterface.gui_White,255);
        ...
        for (int idx = 0; idx <axisVals.length;++idx) {
            ...
            ri.drawLine(xVal, -tic, 0, xVal, tic, 0);    
            //draw line to other side
            ri.setColorValStroke(IGraphicsAppInterface.gui_White,50);
            ri.drawLine(xVal, -frameDims[3], 0, xVal, tic, 0);    
            //draw text for display
    ...
    public final void drawMe(IGraphicsAppInterface ri, boolean isMulti) {
        ri.setFill(fillClr, fillClr[3]);
        ri.setStroke(strkClr, strkClr[3]);
        _drawCurve(ri,isMulti ? frameDims[2] : 0);
    }//drawMe

    protected abstract void _drawCurve(IGraphicsAppInterface ri, float offset);
