...
public class floatTrajAnalyzer extends baseAnalyzer {
    ...
    @Override
    protected void drawSingleSummary(IGraphicsAppInterface ri, String[] mmntDispLabels, baseProbSummary smryRaw, float txtLineYDisp, float ltrMult) {
        myProbSummary_Flts smry = ((myProbSummary_Flts)smryRaw);
        TreeMap<String,String> smryStrings = smry.summaryStringAra("A");
        ri.pushMatState();
        showOffsetText_RightSideMenu(ri,ri.getClr(IGraphicsAppInterface.gui_Black, 255), smryStrings.get("summaryName"));
        for(int i=0;i<mmntDispLabels.length;++i) {
            showOffsetText_RightSideMenuAbs(ri, ri.getClr(IGraphicsAppInterface.gui_DarkBlue, 255), ltrMult*3.5f, smryStrings.get(mmntDispLabels[i]));
        }            
        ri.popMatState();
    ...
    protected final void drawSingleSmryGraphMinMaxLbls(IGraphicsAppInterface ri, int smryIdx, float ltrMult) {
        ...
        drawSingleMinMaxTxt(ri, IGraphicsAppInterface.gui_Black, "Min/Max ["+ String.format(frmtStr,perSmry_MinMax[0])+", " + String.format(frmtStr,perSmry_MinMax[1])+"]",ltrMult);
