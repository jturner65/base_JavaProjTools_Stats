...
public abstract class baseVecTrajAnalyzer extends baseAnalyzer {
    ...
    @Override
    protected final void drawSingleSummary(IGraphicsAppInterface ri, String[] mmntDispLabels, baseProbSummary smryRaw, float txtLineYDisp, float ltrMult) {
        ...
        for(int row=0;row<smryStrings.length;++row) {
            ...
            ri.pushMatState();
            showOffsetText_RightSideMenu(ri,ri.getClr(IGraphicsAppInterface.gui_Black, 255), smryStrings[row].get("summaryName"));
            for(int i=0;i<mmntDispLabels.length;++i) {
                showOffsetText_RightSideMenuAbs(ri, ri.getClr(IGraphicsAppInterface.gui_DarkBlue, 255), ltrMult*3.5f, smryStrings[row].get(mmntDispLabels[i]));
            }            
            ri.popMatState();
    ...
    @Override
    protected final void drawSingleSmryGraph(IGraphicsAppInterface ri, String[] mmntDispLabels, int smryIdx, float[] graphRect, float ltrMult) {
        ...
        for(int sDim = 0; sDim < perSmry_Traj.length;++sDim) {
            ri.pushMatState();            
            //drawSingleTraj(IGraphicsAppInterface ri, int[] clr, float[] trajRect, float[] minMax, float[] trajElems, float widthPerElem)
            drawSingleTraj(ri, trajClrs[sDim], graphRect, perSmry_MinMax[sDim], perSmry_Traj[sDim],widthPerElem);
            ri.popMatState();
    ...
    @Override
    protected final void drawSingleSmryGraphMinMaxLbls(IGraphicsAppInterface ri, int smryIdx, float ltrMult) {
        float[][] perSmry_MinMax = perSummaryMinMax[smryIdx];
        ri.scale(.8f);
        float newLtrMult = 4.5f;
        //(IGraphicsAppInterface ri, int clrLabel, String txt, float ltrMult)
        drawSingleMinMaxTxt(ri, IGraphicsAppInterface.gui_Red, "["+ String.format(frmtStr,perSmry_MinMax[0][0])+", " + String.format(frmtStr,perSmry_MinMax[0][1])+"]",newLtrMult);
        drawSingleMinMaxTxt(ri, IGraphicsAppInterface.gui_DarkGreen, "["+ String.format(frmtStr,perSmry_MinMax[1][0])+", " + String.format(frmtStr,perSmry_MinMax[1][1])+"]",newLtrMult);
        drawSingleMinMaxTxt(ri, IGraphicsAppInterface.gui_DarkBlue, "["+ String.format(frmtStr,perSmry_MinMax[2][0])+", " + String.format(frmtStr,perSmry_MinMax[2][1])+"]",newLtrMult);
        if(perSmry_MinMax.length > 3) {
            drawSingleMinMaxTxt(ri, IGraphicsAppInterface.gui_Magenta, "["+ String.format(frmtStr,perSmry_MinMax[3][0])+", " + String.format(frmtStr,perSmry_MinMax[3][1])+"]",5.0f);
        }
    }
