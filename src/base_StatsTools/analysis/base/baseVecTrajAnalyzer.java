...
public abstract class baseVecTrajAnalyzer extends baseAnalyzer {
	...
	@Override
	protected final void drawSingleSummary(IRenderInterface ri, String[] mmntDispLabels, baseProbSummary smryRaw, float txtLineYDisp, float ltrMult) {
	...
	protected final void drawSingleSmryGraphMinMaxLbls(IRenderInterface ri, int smryIdx, float ltrMult) {
		...
		drawSingleMinMaxTxt(ri, IRenderInterface.gui_Red, "["+ String.format(frmtStr,perSmry_MinMax[0][0])+", " + String.format(frmtStr,perSmry_MinMax[0][1])+"]",newLtrMult);
		drawSingleMinMaxTxt(ri, IRenderInterface.gui_DarkGreen, "["+ String.format(frmtStr,perSmry_MinMax[1][0])+", " + String.format(frmtStr,perSmry_MinMax[1][1])+"]",newLtrMult);
		drawSingleMinMaxTxt(ri, IRenderInterface.gui_DarkBlue, "["+ String.format(frmtStr,perSmry_MinMax[2][0])+", " + String.format(frmtStr,perSmry_MinMax[2][1])+"]",newLtrMult);
		if(perSmry_MinMax.length > 3) {
			drawSingleMinMaxTxt(ri, IRenderInterface.gui_Magenta, "["+ String.format(frmtStr,perSmry_MinMax[3][0])+", " + String.format(frmtStr,perSmry_MinMax[3][1])+"]",5.0f);
		}
	}
