...
public abstract class baseAnalyzer {
     ...
     * @param txt
     */
    protected final void showOffsetText_RightSideMenuAbs(IGraphicsAppInterface ri, int[] tclr, float dist,  String txt) {
        ri.setFill(tclr,tclr[3]);ri.setStroke(tclr,tclr[3]);
        ri.showText(txt,0.0f,0.0f,0.0f);
        ri.translate(dist, 0.0f,0.0f);    
    }
    
    protected final void showOffsetText_RightSideMenu(IGraphicsAppInterface ri, int[] tclr, String txt) {
        ri.setFill(tclr,tclr[3]);ri.setStroke(tclr,tclr[3]);
        ri.showText(txt,0.0f,0.0f,0.0f);
        ri.translate(ri.getTextWidth(txt)*1.1f, 0.0f,0.0f);        
    }
    protected void drawSingleMinMaxTxt(IGraphicsAppInterface ri, int clrLabel, String txt, float ltrMult) {
        showOffsetText_RightSideMenu(ri,ri.getClr(clrLabel, 255), txt);            
    }
    
    
    
    public void drawAnalyzerData(IGraphicsAppInterface ri, String[] mmntDispLabels, float[] trajWinDims, String name) {
        float yDisp = trajWinDims[3];
        ri.pushMatState();        
        ri.translate(5.0f, yDisp, 0.0f);
            showOffsetText_RightSideMenu(ri,ri.getClr(IGraphicsAppInterface.gui_Black, 255), name);
        ri.popMatState();
        ri.pushMatState();
     ...
     * @param name
     */
    public void drawAnalyzerGraphs(IGraphicsAppInterface ri, String[] mmntDispLabels, float[] trajWinDims, String name) {
        float yDisp = trajWinDims[3];
    ...
    protected void drawAllSummaryInfo(IGraphicsAppInterface ri, String[] mmntDispLabels, float txtLineYDisp, float perDispBlockWidth) {//
        ...
        for(int i=0;i<summaries.length;++i) {        //per summary - single summary for pos, vel, accel, jerk, etc of each point
            ...
            ri.pushMatState();
                showOffsetText_RightSideMenu(ri,ri.getClr(IGraphicsAppInterface.gui_Black, 255), statDispLabels[i]);
                for(int j=0;j<mmntDispLabels.length;++j) {        showOffsetText_RightSideMenuAbs(ri,ri.getClr(IGraphicsAppInterface.gui_DarkBlue, 255), mult*3.5f, mmntDispLabels[j]);}
            ri.popMatState();            
    ...
    protected abstract void drawSingleSummary(IGraphicsAppInterface ri, String[] mmntDispLabels, baseProbSummary smryRaw, float txtLineYDisp, float ltrMult);
     ...
     */
    protected final void drawAllGraphInfo(IGraphicsAppInterface ri, String[] mmntDispLabels, float[] graphRect, float txtLineYDisp, float perDispBlockWidth) {
        float mult = (perDispBlockWidth)/((mmntDispLabels.length + 1) * 3.1f);
        ri.pushMatState();
        if(mult < 17) {ri.scale(1.0f,.93f,1.0f);}
        for(int i=0;i<summaries.length;++i) {        //per summary - single summary for pos, vel, accel, jerk, etc of each point
    ...
    protected abstract void drawSingleSmryGraphMinMaxLbls(IGraphicsAppInterface ri, int smryIdx, float ltrMult);
    protected abstract void drawSingleSmryGraph(IGraphicsAppInterface ri, String[] mmntDispLabels, int smryIdx, float[] graphRect,  float ltrMult);
     ...
     */
    protected void drawSingleTraj(IGraphicsAppInterface ri, int[] clr, float[] trajRect, float[] minMax, float[] trajElems, float widthPerElem) {
        ri.pushMatState();
     ...
     * @param widthPerElem how far to displace between each element in trajElems
     */
    protected void drawSingleTraj(IGraphicsAppInterface ri, int[] clr, double[] trajRect, double[] minMax, double[] trajElems, double widthPerElem) {
        ri.pushMatState();
        ri.translate(0.0f,trajRect[3],0.0f);
        ri.setStroke(clr[0],clr[1],clr[2],clr[3]);
        ri.setStrokeWt(1.0f);
